...
| 투자금변경금 | −sum(전체 투자 항목의 deposit 값) |
...
> 투자금변경금: 입금(deposit > 0)은 현금에서 나가므로 음수. 인출(deposit < 0)은 현금으로 들어오므로 양수. 즉 `투자금변경금 = -deposit`. 모든 투자 카테고리(주식, 예금/채권 등)의 deposit이 대상.
...
| 투자 입금/인출 (전체 카테고리) | 가계부 투자금변경금에 자동 반영 (저장 없이 파생) |
